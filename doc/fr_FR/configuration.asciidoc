...
Toutefois, Cozytouch offre la possibilité de passer par un bridge pour pouvoir contràŽler ces radiateurs.
Une application sur smartphone a été développée. Celle-ci envoie les commandes à  un serveur Cozytouch.
Ensuite, le bridge va requàªter ce serveur Cozytouch pour connaitre les commandes qu'il doit envoyer aux radiateurs.
