...
class CozytouchAtlanticHotWaterMBL extends AbstractCozytouchDevice
{
    const cold_water = 15;
	...
	const DISPLAY = [
		...
		CozyTouchStateName::CTSN_MBLDHWMODE=>[4,0,1],
		CozyTouchStateName::CTSN_MBLMIDDLETEMP=>[5,0,0],
	...
	public static function refresh_boost($eqLogic) 
    {
		...
		$cmd = Cmd::byEqLogicIdAndLogicalId($eqLogic->getId(),$deviceURL.'_'.CozyTouchStateName::CTSN_MBLDHWBOOSTMODE);
		if (is_object($cmd)) {
			$value=$cmd->execCmd();
        }
	...
	{
		log::add('cozytouch', 'debug', __('Boost : ', __FILE__).$device_url. "(".$value.")");
		if($value==1)
		{
			$cmds = array(
				array(
